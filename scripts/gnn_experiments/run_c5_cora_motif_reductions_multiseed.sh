...
DB=${DB:-/tmp/mdb_c5_cora_motif_reductions_multiseed}
PORT=${PORT:-7909}
...
LOG_DIR=${LOG_DIR:-logs/C5_query_algebra/cora_motif_reductions_multiseed}
mkdir -p "$LOG_DIR"
...
echo "==== C5 Cora Motif Reductions Multi-Seed ====" | tee "$SUMMARY_TXT"
...
echo "variant,seed,rows,edges,projectedNodes,totalBatches,trainBatches,validationBatches,testBatches,uniqueNodes,bestValAccuracy,testAccuracy,trainSeconds,dbSize" > "$RESULTS_CSV"
...
project_and_prepare_variant() {
  ...
  if [[ -z "$where_clause" ]]; then
    count_query="$match_clause RETURN COUNT($edge_var) AS rows"
    project_query="$match_clause RETURN PROJECT('$graph_name' INCLUDE LABELS INCLUDE PROPERTIES)"
  else
    count_query="$match_clause WHERE $where_clause RETURN COUNT($edge_var) AS rows"
    project_query="$match_clause WHERE $where_clause RETURN PROJECT('$graph_name' INCLUDE LABELS INCLUDE PROPERTIES)"
  fi

  local rows_result
  rows_result=$(query "$count_query")
  echo "$rows_result" | tee "$LOG_DIR/${variant}_01_rows.log" | tee -a "$SUMMARY_TXT"
  ...
  local edges_result
  edges_result=$(query "USE $graph_name MATCH (n:Paper)-[r:CITES]->(m:Paper) RETURN COUNT(r) AS projectedEdges")
  echo "$edges_result" | tee "$LOG_DIR/${variant}_04_projected_edges.log" | tee -a "$SUMMARY_TXT"
...
run_variant_seed() {
  ...
  local rows
  rows=$(tail -n +2 "$LOG_DIR/${variant}_01_rows.log" | head -n 1)
  ...
  echo "$variant,$seed,$rows,$edges,$projected_nodes,$total_batches,$train_batches,$val_batches,$test_batches,$unique_nodes,$best_val,$test_acc,$train_seconds,$db_size" >> "$RESULTS_CSV"
}

MATCH_1HOP='MATCH (n:Paper)-[r:CITES]->(m:Paper)'
MATCH_2HOP='MATCH (a:Paper)-[r1:CITES]->(b:Paper)-[r2:CITES]->(c:Paper)'
MATCH_MOTIF_OUT='MATCH (a:Paper)<-[r1:CITES]-(b:Paper)-[r2:CITES]->(c:Paper)'
MATCH_MOTIF_IN='MATCH (a:Paper)-[r1:CITES]->(b:Paper)<-[r2:CITES]-(c:Paper)'
...
project_and_prepare_variant \
  ...
  "cora_c5_2hop_middle_train_val_ms" \
  "$MATCH_2HOP" \
  "$MIDDLE_TRAIN_VAL" \
  "r2"
...
project_and_prepare_variant \
  "motif_in_center" \
  "cora_c5_motif_in_center_ms" \
  "$MATCH_MOTIF_IN" \
  "$MIDDLE_TRAIN_VAL" \
  "r2"

for seed in $SEEDS; do
  run_variant_seed "full_declarative" "cora_c5_full_ms" "$seed"
  run_variant_seed "train_val_touch" "cora_c5_train_val_touch_ms" "$seed"
  run_variant_seed "2hop_middle_train_val" "cora_c5_2hop_middle_train_val_ms" "$seed"
  run_variant_seed "motif_out_center" "cora_c5_motif_out_center_ms" "$seed"
  run_variant_seed "motif_in_center" "cora_c5_motif_in_center_ms" "$seed"
done
...
echo "==== PASS C5 Cora Motif Reductions Multi-Seed ====" | tee -a "$SUMMARY_TXT"
