#!/usr/bin/env bash
# T1 SMALL: papers100M fanout=[10,10] batchSize=512.
# Gate: T0 showed loss decreasing. Now check if val_acc actually moves
# above 0.0776 baseline with richer 2-hop neighborhoods.
# Total wall-clock target ~50 min.
set -euo pipefail
...
PORT="${PORT:-29961}"
OUT_DIR="${OUT_DIR:-$HOME/Desktop/spec13_papers100m_e2e/post_pop_os/36_t1_small_f10_b512}"
mkdir -p "$OUT_DIR"

SAMPLE_NAME="papers100M_t1_f10_b512"
...
echo
echo "=== Phase 1: gnn_offline_sample [10,10] batch 512 ==="
P1_START=$(date +%s)
curl -s --max-time 1800 -X POST -H 'Content-Type: application/gql' \
...
echo
echo "=== Phase 3: gnn_train 5 epochs fanout [10,10] batch 512 ==="
P3_START=$(date +%s)
...
echo
echo "Gate: val_acc should reach > 0.10 by epoch 5."
