...
One commit per logical fix — never bulk commits grouping unrelated changes. Use `<type>(<scope>): <summary>` format with a detailed body (2-5 lines) explaining *what* and *why*. Add files explicitly by name (never `git add -A`). Types: fix, feat, refactor, chore, docs, test. Scopes: gnn, gql, hnsw, build, etc.
