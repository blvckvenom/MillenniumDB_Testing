...
CORA_NPY="data/example/gql/cora/cora_features.npy"
DB_DIR="/tmp/mdb_cora_b6_query_combined"
LOG="/tmp/mdb_cora_b6_query_combined.log"
PORT=19904

FEATURE_NAME="cora_query_combined_features_zscore"
BASE_FEATURE_NAME="node_features"
GRAPH_NAME="cora_b6_query_combined_graph"
SAMPLE_NAME="cora_b6_query_combined_graph_s"
OUTPUT_DIR="cora_b6_query_combined_train"
SERVER_PID=""

FEATURE_QUERY="MATCH (n:Paper)
RETURN n._id AS node_id,
       toFloat(outDegree(n)) AS x_out_degree,
       toFloat(inDegree(n)) AS x_in_degree,
       toFloat(degree(n)) AS x_total_degree,
       toFloat(outDegree(n) - inDegree(n)) AS x_degree_balance,
       toFloat(outDegree(n) * outDegree(n)) AS x_out_degree_sq,
       toFloat(inDegree(n) * inDegree(n)) AS x_in_degree_sq
ORDER BY n"
...
check_prereqs() {
  ...
  [[ -f "$CORA_NPY" ]] || fail "Missing Cora tensor file: $CORA_NPY"
}
...
echo "Importing Cora with tensors into ${DB_DIR}"
"$MDB" import "$CORA_GQL" "$DB_DIR" --with-tensors "$CORA_NPY"
...
run_gql "Create rich query features appended to node_features" \
"CALL gnn_create_features_from_query('${FEATURE_NAME}', '${FEATURE_QUERY}', {normalize:'zscore',appendToFeature:'${BASE_FEATURE_NAME}'})
YIELD featureName,nodeCount,featureDim,baseFeatureDim,queryFeatureDim,fmatPath,rmapPath,normalized,appendedToFeature
RETURN *"
