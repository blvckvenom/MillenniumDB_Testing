...
EdgePropertyPlan::EdgePropertyPlan(Id object, Id key, Id value) :
    object(object),
    key(key),
    value(value),
...
{
    if (gql_model.catalog.edge_key2total_count.count(key.get_OID().get_value())) {
        return gql_model.catalog.edge_key2total_count[key.get_OID().get_value()];
    }
    return 0;
}
...
std::unique_ptr<BindingIter> EdgePropertyPlan::get_binding_iter() const
{
    ...
    if (object_assigned) {
        ...
        return std::make_unique<IndexScan<3>>(gql_model.get_edge_key_value(), std::move(ranges));
    } else {
        ...
        // Use protected getter for proper projection context handling
        return std::make_unique<IndexScan<3>>(gql_model.get_key_value_edge(), std::move(ranges));
    }
}
