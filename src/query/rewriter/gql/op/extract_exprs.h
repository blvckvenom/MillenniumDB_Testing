...
class ExtractExprs : public OpVisitor {
public:
    std::unique_ptr<Op> tmp;

private:
    bool graph_pattern_visited = false;
    std::vector<std::unique_ptr<Expr>> tmp_exprs;
    ...
    void visit(OpCall& op) override;
    void visit(OpProject& op) override;
