...
class ExprCosineSimilarity : public Expr {
public:
    std::unique_ptr<Expr> expr1;
    std::unique_ptr<Expr> expr2;

    ExprCosineSimilarity(std::unique_ptr<Expr> expr1, std::unique_ptr<Expr> expr2) :
    ...
    {
        return std::make_unique<ExprCosineSimilarity>(expr1->clone(), expr2->clone());
    }
