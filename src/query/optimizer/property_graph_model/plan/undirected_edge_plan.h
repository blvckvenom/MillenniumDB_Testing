...
class UndirectedEdgePlan : public Plan {
public:
    UndirectedEdgePlan(Id edge, Id from, Id to, VarId direction_var);

    UndirectedEdgePlan(const UndirectedEdgePlan& other) :
        edge(other.edge),
        from(other.from),
        to(other.to),
    ...
    {
        return std::make_unique<UndirectedEdgePlan>(*this);
    }
