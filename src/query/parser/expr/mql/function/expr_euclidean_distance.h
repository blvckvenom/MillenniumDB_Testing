...
class ExprEuclideanDistance : public Expr {
public:
    std::unique_ptr<Expr> expr1;
    std::unique_ptr<Expr> expr2;

    ExprEuclideanDistance(std::unique_ptr<Expr> expr1, std::unique_ptr<Expr> expr2) :
    ...
    {
        return std::make_unique<ExprEuclideanDistance>(expr1->clone(), expr2->clone());
    }
