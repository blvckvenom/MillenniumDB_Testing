#!/usr/bin/env bash
# products_e2e.sh — mid-scale (ogbn-products) end-to-end validation of the GNN pipeline
...
#
# Dataset: data/example/gql/ogbn-products/ogbn-products/ (169,343 nodes, 128 feat, 40 classes,
# OGB time-split train/valid/test). Generate it once with:
#   <conda diskgnn_cu124>/bin/python scripts/gnn_datasets/download_ogb.py \
#       --dataset ogbn-products --output data/example/gql/ogbn-products/ \
#       --ogb-root /home/bfuentes/diskgnn_data/ogb_raw
#
# Exit 0 = PASS.
set -u -o pipefail

REPO=/home/svergara/tesis/MillenniumDB_Testing
MDB="$REPO/build/GNN/bin/mdb"
DSET="$REPO/data/example/gql/ogbn-products/ogbn-products"
GQL="$DSET/ogbn_products.gql"
NPY="$DSET/ogbn_products_features.npy"
PORT=7884
DB="$REPO/outputs/C2_scalability/products_db/run_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$DB"
LOG="$DB/server.log"
SRVPID=""
RC=1
...
cleanup() {
  ...
  echo "[products] keeping DB at $DB"
  exit $RC
}
trap cleanup EXIT INT TERM
say()  { echo "[products] $*"; }
fail() { echo "[products] FAIL: $*" >&2; RC=1; exit 1; }
...
[ -f "$GQL" ] || fail "products gql not found at $GQL (run download_ogb.py first)"
[ -f "$NPY" ] || fail "products features not found at $NPY"
...
t0=$(now)
b=$(gql "CALL graph_project('products_gnn','Node','CONNECTS',{orientation:'UNDIRECTED',includeFeatures:'node_features',labelProperty:'label',splitProperty:'split'}) YIELD graphName,nodeCount,relationshipCount,featureDim,numClasses RETURN *")
...
t0=$(now)
b=$(gql "CALL gnn_offline_sample('products_gnn','products_s',[10,5],{batchSize:512,randomSeed:42,orientation:'UNDIRECTED'}) YIELD totalBatches,trainBatches,validationBatches,testBatches,uniqueNodes,numWorkersUsed RETURN *")
...
# --- 3. materialize ---
say "materialize: skipped (gnn_materialize_batches is deprecated/redundant; using gnn_build_feature_store directly)"

# --- 4. build feature store (GPU L1) ---
t0=$(now)
b=$(gql "CALL gnn_build_feature_store('products_s','node_features',{gpu_budget_mb:4096,cpu_budget_mb:16384}) YIELD l1Nodes,l2Nodes,l3Nodes,l4Nodes RETURN *")
...
# --- 5. train ---
t0=$(now)
b=$(gql "CALL gnn_train('products_s','node_features',{model:'graphsage',hiddenDim:128,epochs:30,lr:0.01,dropout:0.5,patience:10,randomSeed:42}) YIELD testAccuracy,bestValAccuracy,ranEpochs,trainSeconds,useAddrTablesEffective,l1HitRatio RETURN *")
...
off=$(wc -c < "$LOG")
b=$(gql "CALL gnn_build_feature_store('products_s','node_features',{gpu_budget_mb:4096,cpu_budget_mb:16384}) YIELD l1Nodes RETURN *")
