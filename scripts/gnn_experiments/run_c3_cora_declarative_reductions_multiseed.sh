...
DB=${DB:-/tmp/mdb_c3_cora_declarative_reductions_multiseed}
PORT=${PORT:-7900}
...
LOG_DIR=${LOG_DIR:-logs/C3_declarative_projection/cora_reductions_multiseed}
mkdir -p "$LOG_DIR"
...
echo "==== C3 Cora Declarative Reductions Multi-Seed ====" | tee "$SUMMARY_TXT"
...
project_and_prepare_variant \
  "full_declarative" \
  "cora_c3_full_declarative_ms" \
  ""

project_and_prepare_variant \
  "train_touch" \
  "cora_c3_train_touch_ms" \
  'n.split = "train" OR m.split = "train"'

project_and_prepare_variant \
  "train_val_touch" \
  "cora_c3_train_val_touch_ms" \
  'n.split = "train" OR n.split = "val" OR n.split = "valid" OR m.split = "train" OR m.split = "val" OR m.split = "valid"'

for seed in $SEEDS; do
  run_variant_seed "full_declarative" "cora_c3_full_declarative_ms" "$seed"
  run_variant_seed "train_touch" "cora_c3_train_touch_ms" "$seed"
  run_variant_seed "train_val_touch" "cora_c3_train_val_touch_ms" "$seed"
done
...
echo "==== PASS C3 Cora Declarative Reductions Multi-Seed ====" | tee -a "$SUMMARY_TXT"
