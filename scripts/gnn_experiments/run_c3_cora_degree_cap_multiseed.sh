...
DB=${DB:-/tmp/mdb_c3_cora_degree_cap_multiseed}
PORT=${PORT:-7903}
...
LOG_DIR=${LOG_DIR:-logs/C3_declarative_projection/cora_degree_cap_multiseed}
mkdir -p "$LOG_DIR"
...
echo "==== C3 Cora Degree-Cap Multi-Seed ====" | tee "$SUMMARY_TXT"
...
project_and_prepare_variant \
  "full_declarative" \
  "cora_c3_full_degree_baseline_ms" \
  ""

project_and_prepare_variant \
  "degree_cap_5" \
  "cora_c3_degree_cap_5_ms" \
  "degree(n) <= 5 AND degree(m) <= 5"

project_and_prepare_variant \
  "degree_cap_10" \
  "cora_c3_degree_cap_10_ms" \
  "degree(n) <= 10 AND degree(m) <= 10"

project_and_prepare_variant \
  "degree_cap_20" \
  "cora_c3_degree_cap_20_ms" \
  "degree(n) <= 20 AND degree(m) <= 20"

for seed in $SEEDS; do
  run_variant_seed "full_declarative" "cora_c3_full_degree_baseline_ms" "$seed"
  run_variant_seed "degree_cap_5" "cora_c3_degree_cap_5_ms" "$seed"
  run_variant_seed "degree_cap_10" "cora_c3_degree_cap_10_ms" "$seed"
  run_variant_seed "degree_cap_20" "cora_c3_degree_cap_20_ms" "$seed"
done
...
echo "==== PASS C3 Cora Degree-Cap Multi-Seed ====" | tee -a "$SUMMARY_TXT"
