...
class DirectedEdgePlan : public Plan {
public:
    DirectedEdgePlan(Id edge, Id from, Id to, VarId direction_var, ObjectId direction);

    DirectedEdgePlan(const DirectedEdgePlan& other) :
        edge(other.edge),
        from(other.from),
        to(other.to),
    ...
    {
        return std::make_unique<DirectedEdgePlan>(*this);
    }
