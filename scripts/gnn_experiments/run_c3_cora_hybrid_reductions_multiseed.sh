...
DB=${DB:-/tmp/mdb_c3_cora_hybrid_reductions_multiseed}
PORT=${PORT:-7904}
...
LOG_DIR=${LOG_DIR:-logs/C3_declarative_projection/cora_hybrid_reductions_multiseed}
mkdir -p "$LOG_DIR"
...
echo "==== C3 Cora Hybrid Reductions Multi-Seed ====" | tee "$SUMMARY_TXT"
...
run_variant_seed() {
  ...
  local edges
  edges=$(tail -n +2 "$LOG_DIR/${variant}_01_edges.log" | head -n 1)
...
TRAIN_VAL='n.split = "train" OR n.split = "val" OR n.split = "valid" OR m.split = "train" OR m.split = "val" OR m.split = "valid"'
TV_PLUS_D5='n.split = "train" OR n.split = "val" OR n.split = "valid" OR m.split = "train" OR m.split = "val" OR m.split = "valid" OR (degree(n) <= 5 AND degree(m) <= 5)'
TV_PLUS_D10='n.split = "train" OR n.split = "val" OR n.split = "valid" OR m.split = "train" OR m.split = "val" OR m.split = "valid" OR (degree(n) <= 10 AND degree(m) <= 10)'

project_and_prepare_variant \
  "full_declarative" \
  "cora_c3_hybrid_full_ms" \
  ""

project_and_prepare_variant \
  "train_val_touch" \
  "cora_c3_hybrid_train_val_touch_ms" \
  "$TRAIN_VAL"

project_and_prepare_variant \
  "train_val_plus_degree_5" \
  "cora_c3_hybrid_train_val_plus_degree_5_ms" \
  "$TV_PLUS_D5"

project_and_prepare_variant \
  "train_val_plus_degree_10" \
  "cora_c3_hybrid_train_val_plus_degree_10_ms" \
  "$TV_PLUS_D10"

for seed in $SEEDS; do
  run_variant_seed "full_declarative" "cora_c3_hybrid_full_ms" "$seed"
  run_variant_seed "train_val_touch" "cora_c3_hybrid_train_val_touch_ms" "$seed"
  run_variant_seed "train_val_plus_degree_5" "cora_c3_hybrid_train_val_plus_degree_5_ms" "$seed"
  run_variant_seed "train_val_plus_degree_10" "cora_c3_hybrid_train_val_plus_degree_10_ms" "$seed"
done
...
def std(xs):
    if len(xs) <= 1:
        return 0.0
    m = mean(xs)
    return math.sqrt(sum((x - m) ** 2 for x in xs) / (len(xs) - m) if False else sum((x - m) ** 2 for x in xs) / (len(xs) - 1))
...
echo "==== PASS C3 Cora Hybrid Reductions Multi-Seed ====" | tee -a "$SUMMARY_TXT"
