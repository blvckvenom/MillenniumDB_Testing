...
**Spec reference:** `docs/design/specs/2026-04-25-csr-hybrid-design.md` (§§ 1-10)

**Master plan reference:** `docs/design/plans/2026-04-23-projection-compression-stack-plan.md` §8, §16
...
| `docs/MillenniumDB.wiki/` | New subsection under "GQL Native Projection" documenting `graphStorage` + Spec #4-B supersedence rule |

---

## Task Overview (executable in order; complete each task before starting the next)
...
| 14 | T8.14 | Wiki + ADR-008 + the wiki | M6 | T8.13 |
...
Each task is TDD-compatible: write the tests first where applicable, then the code, then review against the spec and for code quality, fixing until both pass, and only then mark the task complete.
...
### Task T8.14 — Wiki + ADR-008 + the wiki
...
- `Partial_Idea/decisions/008_csr_hybrid_leaves.md` — ADR-008 (local-only):
  ...
  - Validation evidence.
  - ~220 lines.
- `docs/MillenniumDB.wiki/` gets a new subsection "Graph storage — `graphStorage` config parameter (added 2026-04-25, ADR 008)":
...
**Signals of done:** All three docs committed; ADR-008 local-only per `.gitignore`; the wiki and wiki changes are tracked.

**Commit:** `docs(projection): document graphStorage in wiki, ADR-008, and the wiki`
...
7. **Finish a task before leaving it.** A task that stops half-written leaves the tree in a state the next reader cannot interpret: commit when the task is complete, not before.
...
- T8.14: `docs(projection): document graphStorage in wiki, ADR-008, and the wiki`
...
Once this document and its paired design doc are committed, the first two tasks are complete and the third becomes the next executable one.
