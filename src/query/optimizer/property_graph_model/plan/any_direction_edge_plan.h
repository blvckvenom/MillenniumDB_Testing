...
class AnyDirectionEdgePlan : public Plan {
public:
    AnyDirectionEdgePlan(Id edge, Id from, Id to, VarId direction_var);

    AnyDirectionEdgePlan(const AnyDirectionEdgePlan& other) :
        edge(other.edge),
        from(other.from),
        to(other.to),
    ...
    {
        return std::make_unique<AnyDirectionEdgePlan>(*this);
    }
