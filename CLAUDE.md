...
**Known limitations (Spec #8-B follow-up — updated 2026-04-24):**
1. `edge_id` is NOT persisted in the v3 layout — `count(e)` queries over CSR_HYBRID projections return inflated counts; GNN flows that only need topology are unaffected. **Spec #8-B task #1 in progress (edge_id stream being added).**
...
4. EmbeddingWriter non-seed inference has O(chunk_idx) growth per chunk. For arxiv-scale (15 chunks × 2048) sums to ~23 min. For products-scale (~500 chunks) projects hours. **Spec #9 candidate (GPU-native EmbeddingWriter). Workaround: `inferenceBatchSize: 2048+` in `gnn_train` config; progress log added in commit `b8bdf10f`.**
